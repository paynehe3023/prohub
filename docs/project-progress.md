...
## 四十、音乐来源按钮颜色与圆角样式修复（2026-09-01）

- 修复正式页面中“歌曲源”按钮样式未完全统一的问题，歌曲源按钮现在与下载按钮使用相同的圆角、边框、内边距和过渡效果。
- 网易云音乐来源使用红色圆形标识和红色按钮强调色。
- QQ 音乐来源使用绿色圆形标识和绿色按钮强调色。
- Shazam 来源使用蓝色圆形标识和蓝色按钮强调色。
- 下载按钮始终使用固定蓝色图标和蓝色按钮样式，不跟随音乐来源变化。
- 前端 `npm run build` 已通过；Docker 正在重新构建，完成后需刷新正式网页确认静态资源已更新。

## 三十九、视频音乐识别与上传优化方案（2026-09-01）

- **资源优化结论**：仅提取音乐时无需把视频文件传到服务端后再处理；浏览器端可先将音频轨道抽取后上传，但需要浏览器支持 WebCodecs/MediaSource，兼容性和音频编码处理复杂。当前 Worker 仍从视频抽取音频，下一步适合增加“仅音乐”模式：服务端接收音频文件，跳过视频上传和视频解码。
- **上传速度优化方向**：继续采用分片上传只是降低单请求超时；更快的方案是浏览器端压缩/抽取音频、音频直传 Worker 或对象存储、并行上传分片、断点续传和局域网直连。当前实现先保留稳定的串行分片流程，避免并发写入同一临时文件造成损坏。
- **识别来源扩展**：Shazam 仍作为音频指纹识别入口；新增网易云音乐和 QQ 音乐文本检索适配。由于两家平台没有稳定公开的音频指纹接口，当前先使用 Shazam 返回的歌名和作者检索国内平台，获取中文歌名、作者、专辑和歌曲链接，不能把普通文本搜索误称为独立音频指纹识别。
- **来源优先级**：识别成功后优先采用网易云音乐中文结果，其次 QQ 音乐结果，最后回退 Shazam 原始结果；结果增加 `source` 字段，前端按来源显示网易云红色图标、QQ 音乐绿色图标，Shazam 使用蓝色图标。
- **交互调整**：歌曲源改为与下载按钮一致的圆角按钮样式；歌曲源按钮图标按识别来源着色，下载按钮图标始终保持固定蓝色，不随音乐来源变化。
- **验证**：前端 `npm run build`、Node 语法检查、Worker 容器内 `python -m compileall -q /app` 均通过；Docker 重建和真实国内平台接口联调仍需后续验证。
