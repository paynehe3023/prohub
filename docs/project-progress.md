...
- 新增纯前端路由 `/tools/image-studio`，入口已加入首页工具清单，替换原来的“图片压缩转换”占位工具。
- 组件文件：`src/src/views/tools/ImageStudio.vue`。
- 读取图片时保留 `naturalWidth`、`naturalHeight` 和原始文件体积；预览 Canvas 使用原始物理尺寸，CSS 只负责等比显示。
- 已实现指定 KB 压缩、二分查找 JPEG 质量、标准尺寸预设、物理像素裁剪框、动态平铺水印、马赛克/黑色遮罩、JPG/PNG/WebP 转换和 Canvas EXIF 清洗。
- 已实现多图纵向/横向拼接；纵向拼接以第一张图片的物理宽度为基准；九宫格按居中正方形切割并支持 ZIP 下载。
- HEIC/HEIF 导入使用前端 `heic2any`，九宫格 ZIP 使用前端 `jszip`，不依赖后端。
- 依赖补充：`heic2any`、`jszip`、`typescript`；需要重新安装前端依赖后再进行构建和浏览器测试。
